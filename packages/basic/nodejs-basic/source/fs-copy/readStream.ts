...
export class ReadStream extends EventEmitter {
  ...
  read() {
    ...
    const readLength = this.end ? Math.min(this.end - this.pos + 1, this.highWaterMark) : this.highWaterMark;

    fs.read(this.fd, buffer, 0, readLength, this.pos, (err, bytesRead, buffer) => {
